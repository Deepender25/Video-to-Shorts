...
import { uploadVideo, generateSubtitles, exportVideo, transcribeClipOnDemand } from './services/api';
...
const App: React.FC = () => {
  ...
  // --- Initial URL parsing for direct editor mode ---
  useEffect(() => {
    const initFromPath = async () => {
      const pathParts = window.location.pathname.split('/').filter(Boolean);
      // Expected path: /editor/[job_id]/[filename]
      if (pathParts[0] === 'editor' && pathParts.length >= 3) {
        const jobId = pathParts[1];
        const filename = decodeURIComponent(pathParts.slice(2).join('/')); 
        
        try {
          setAppState(AppState.GENERATING);
          const subs = await transcribeClipOnDemand(jobId, filename);
          
          setJobId(jobId);
          setCurrentFilename(filename);
          // Assuming backend serves video cleanly via this URL
          setVideoUrl(`/api/download/${jobId}/${filename}?dl=0`);
          initSubtitles(subs);
          setAppState(AppState.EDITOR);
        } catch (e) {
          console.error("Failed to load preprocessed clip", e);
          alert("Failed to load clip data from the backend.");
