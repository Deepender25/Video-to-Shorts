...
export function useHistory<T>(initialPresent: T) {
    ...
    // Helper to init without pushing to history (e.g. initial load)
    const init = useCallback((initialValue: T) => {
        setState({
            past: [],
    ...
    return { state: state.present, set, undo, redo, canUndo, canRedo, init };
}
