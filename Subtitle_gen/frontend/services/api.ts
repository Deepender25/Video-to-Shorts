...
export const transcribeClipOnDemand = async (jobId: string, filename: string): Promise<Subtitle[]> => {
    // Hits the backend endpoint to run whisper transcription dynamically
    const response = await fetch(`${API_BASE}/api/transcribe_clip/${jobId}/${filename}`);
    
    if (!response.ok) {
        const error = await response.json().catch(() => ({}));
        throw new Error(error.error || 'Failed to generate transcription for clip');
    }
