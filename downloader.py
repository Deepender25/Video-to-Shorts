    ...
    for attempt in range(MAX_DOWNLOAD_RETRIES):
        try:
            ...
            if "Video unavailable" in stderr or "Private video" in stderr:
                raise ValueError(
                    "This video is unavailable or private. Please try a different URL."
                )
            if "is not a valid URL" in stderr or "Unsupported URL" in stderr:
                raise ValueError(
                    "Invalid URL. Please provide a valid YouTube video link."
                )
            if "Sign in to confirm" in stderr or "age" in stderr.lower():
                raise ValueError(
                    "This video requires age verification and cannot be processed."
                )
            ...
            if attempt < MAX_DOWNLOAD_RETRIES - 1:
                ...
                continue
            raise RuntimeError(
                "Timed out getting video info. The video may be too long or YouTube is slow."
            )

    raise RuntimeError("Failed to get video info after all retries.")


def _download_video_cli(url: str, output_dir: str, video_id: str) -> str | None:
...
def _download_captions_cli(url: str, output_dir: str, video_id: str) -> str | None:
    """
    ...
    for lang in SUBTITLE_LANGS:
        srt = _try_caption_download(
            url, output_dir, video_id, lang, auto=False
        )
        if srt:
    ...
    for lang in SUBTITLE_LANGS:
        srt = _try_caption_download(
            url, output_dir, video_id, lang, auto=True
        )
        if srt:
...
def download_video(url: str, output_dir: str) -> dict:
    ...
    # ── Download video ──
    video_path = _download_video_cli(url, output_dir, video_id)
    ...
    srt_path = _download_captions_cli(url, output_dir, video_id)
    if srt_path is None:
        raise ValueError(
